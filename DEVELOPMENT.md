...
このドキュメントは、`cmake-ninja-docker-image` の開発・リリース時に使用するメモです。
...
```sh
docker build -t cmake-ninja:local .
```

ビルド後にCMakeを確認：

```sh
docker run --rm cmake-ninja:local cmake --version
```

Ninjaを確認：

```sh
docker run --rm cmake-ninja:local ninja --version
```
...
docker buildx build \
  ...
  -t ghcr.io/higma-container/cmake-ninja:v0.8 \
  --push .
```
...
```text
ghcr.io/higma-container/cmake-ninja:v0.8
...
```yaml
cache-from: type=gha,scope=cmake-ninja-amd64
cache-to: type=gha,mode=max,scope=cmake-ninja-amd64
```

arm64：

```yaml
cache-from: type=gha,scope=cmake-ninja-arm64
cache-to: type=gha,mode=max,scope=cmake-ninja-arm64
```
...
test.yaml
   │
   ├── amd64 ──→ cmake-ninja-amd64 cache
   │
   └── arm64 ──→ cmake-ninja-arm64 cache
                         │
                         ↓
                    release.yaml
```
...
```text
higma-container/cmake-ninja-docker-image
```
...
```text
ghcr.io/higma-container/cmake-ninja
```
